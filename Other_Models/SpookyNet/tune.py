...
if __name__ == "__main__":
    ...
    calc.spookynet = torch.load("./weight/20221207_self_trained_3.pt")
    calc.spookynet.train()

    opt = Adam(calc.spookynet.parameters(), lr=1e-5)
    ...
    with open("./training_process/20221207_tuning_1.txt", "w+") as f:
        for i, pkl in enumerate(pkls):
            pkl = pickle.load(open(pkl, "rb"))
            train_loss, train_record = train(_epoch=i, _calc=calc, _opt=opt, _data=pkl, _num=64)
            test_loss, test_record = test(_epoch=i, _calc=calc, _data=pkl, _num=100)
            torch.save(calc.spookynet, "./weight/20221207_tuning_1_{}_{}.pt".format(train_loss, test_loss))
