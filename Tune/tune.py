...
def preprocess(file_path):
    if os.path.isdir(file_path):
        ## a path is given with multiple files
        all_names = os.listdir(file_path)
        all_names.sort()

        if file_path.endswith("/"):
            preprocessed_file_path = file_path[:-1] + "_preprocessed/"
            GOODLE_input_file_path = file_path[:-1] + "_GOODLE_input/"
        else:
            preprocessed_file_path = file_path + "_preprocessed/"
            GOODLE_input_file_path = file_path + "_GOODLE_input/"

        pathlib.Path(preprocessed_file_path).mkdir(parents=True, exist_ok=True)
        pathlib.Path(GOODLE_input_file_path).mkdir(parents=True, exist_ok=True)

        # for i in tqdm(range(len(all_names)), ncols=80):
        for i in range(len(all_names)):
            print("#### XYZ FILE ####", file_path + all_names[i])
            xyz_to_GOODLE_input(file_path + all_names[i], GOODLE_input_file_path + all_names[i])
            
            ks, true_bands, positions, accessible_R, Rx, Ry, Rz, atomic_numbers = read_file(GOODLE_input_file_path + all_names[i])
            ks = torch.tensor(ks)
            ...
            np.save(os.path.join(preprocessed_file_path, all_names[i][:-4]), [positions, atomic_numbers, distance_matrices, ori_dist, dist_xs, dist_ys, dist_zs, kRs, accessible_R, ks.numpy(), true_bands])
            print("#### NPY FILE ####", preprocessed_file_path + all_names[i][:-4] + ".npy")

        return preprocessed_file_path

    else:
        ## only one file
        ks, true_bands, positions, accessible_R, Rx, Ry, Rz, atomic_numbers = read_file(file_path)
        ks = torch.tensor(ks)
        distance_matrices, dist_xs, dist_ys, dist_zs, kRs, ori_dist = cal_dist(positions, accessible_R, ks, Rx, Ry, Rz)
        positions = np.array(positions)
        accessible_R = np.array(accessible_R)
        ...
        np.save(file_path[:-4], [positions, atomic_numbers, distance_matrices, ori_dist, dist_xs, dist_ys, dist_zs, kRs, accessible_R, ks.numpy(), true_bands])
        return file_path[:-4] + ".npy"

if __name__ == "__main__":
    # preprocess("./all_data_qm9/")
